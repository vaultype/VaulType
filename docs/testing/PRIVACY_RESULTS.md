...
| Sparkle update check | `vaultype.github.io` (path: `/VaulType/appcast.xml`) | Configured by `SUEnableAutomaticChecks` in `Info.plist` | Periodic (Sparkle default: 24 hours) | No (background) |
...
- `tcpdump` capture may show failed connection attempts to `raw.githubusercontent.com` and `vaultype.github.io` — these are expected and acceptable (they will fail immediately). They must not block the UI.
...
```xml
<key>SUFeedURL</key>
<string>https://vaultype.github.io/VaulType/appcast.xml</string>
...
**Monitoring command:**
```bash
sudo tcpdump -i en0 host vaultype.github.io -w /tmp/sparkle-check.pcap &
...
**Expected Result:**
- One HTTPS GET request to `vaultype.github.io/VaulType/appcast.xml`
...
- No connections to any host other than `vaultype.github.io` and its CDN

**Pass Criteria:**
- [ ] Only `vaultype.github.io` is contacted during the update check
...
- [ ] If `vaultype.github.io` is unreachable, Sparkle fails silently (no user-visible error, no crash)
...
| Hostname | Reason |
|---|---|
| `vaultype.github.io` | Sparkle appcast check |
...
5. **Sparkle stays on declared host** — Scenario 6 passes: update check contacts only `vaultype.github.io`.
